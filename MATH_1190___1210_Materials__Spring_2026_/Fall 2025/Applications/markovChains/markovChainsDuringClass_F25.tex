...
\item Each column sums up to $1$. This reflects the reality that when starting in state $i$, the probability that we move to onee of states $1$ through $5$ must sum to be $1$.
}
}
\newpage
...
%\addplot[ybar, bar shift=0,   bar width=24pt,  fill=blue!50] plot coordinates { (1,.25)  (2,0) (3,0.5) (4,0) (5,0.25) };
\end{axis}
\end{tikzpicture}
}
...
\end{center}
\tasksA{2}{
\task Create the transition matrix associated with the transition diagram here and in Desmos. \vs{.3}\\
$P=$


\task Let's assume that at the beginning of our model $95\%$ of the population is Susceptible, $4\%$ are Infected and $1\%$ have recovered. Find the initial state vector, Since we are already using $S$ to represent the Susceptible part of the population, we'll use $V$ for our state vector.  \vs{.3}\\
$V=$
\vs{1.5}
