...
\def\TypeNum{Tecovas Boot Co. Pre-Class \red{Solutions}}
...
\al{
R&=p\cdot Q\\
&=p\cdot z\cdot L^{\alpha}\\
&=305.83\cdot 0.75 \cdot L^{2/3}\\
&=229.3725 \cdot L^{2/3}\\
R(200)&=\$7844.43\\
R(1500)&=\$30,056.30
}
...
\al{
P&=R-C\\
&=p\cdot z\cdot L^{\alpha}-w\cdot L\\
&=229.3725 \cdot L^{2/3}-21.50\cdot L\\
P(200)&=\$3544.43\\
P(1500)&=\$-2193.70 
}
Note that $\ds P(1500)=\$-2193.70$ means Tecovas is losing money when using $1500$ labor hours.
}
\vs{2}
}
\vfill
...
\al{
P&=p\cdot z\cdot L^{\alpha}-w\cdot L\\
&=305.83 \cdot z\cdot 200^{2/3}-4300\\
&=10459.24\cdot z-735.28 \qquad\text{(coefficients rounded to $2$ decimal places)}\\
\frac{dP}{dz}&=10459.24>0\\
}
So the profit will increase when productivity increases.
}
\vs{1.25}
...
\al{
P&=p\cdot z\cdot L^{\alpha}-w\cdot L\\
&=305.83 \cdot 0.75\cdot 200^{\alpha}-21.50\cdot 200\\
&=229.3725\cdot 200^{\alpha}- 4300\\
\frac{dP}{d\alpha}&=229.3725\cdot 200^{\alpha} \cdot \ln(200)\\
&=1215.29 \cdot 200^{\alpha} \qquad\text{(rounded to two decimal places)}\\
&>0\qquad \text{(since $200^{\alpha}>0$)}
}
So the profit will increase when elasticity of labor output  increases.
}
}
