...
\lg Students will...
\itemI{
\item Engage mathematics in a real context of Keeling Curve data.
\item Meaningfully use technology to create functions of best fit/trend lines to model Keeling Curve Data
\item Meaningfully use technology to estimate, calculate,  and visualize Average Rate of Change and Instantaneous Rate of change.
\item be more motivated by Purpose mindset\footnote{See Chris Hulleman and Motivate Lab's GPS workshop from August 2024}
\item Work in teams to solve more challenging problems.
}

\hand
\itemI{
\item {\em Pre-class} Students will complete two exercises H5P exercise
\itemI{
\item H5P: Students will create two different linear functions from data related to the Keeling Curve. 
\item Written: The pre-class handout has first two questions of During-class handout.  Students will answer these two questions so they can get past any technical hurdles of logging into and using Desmos to generate a trend line. 
}  
\item {\em During class} Question $3$: Students use Desmos to create 3 additional functions of best fit (quadratic, exponential and logarithmic) and assocated $R^2$ values.
\item {\em During class} Question $4$: Students to interpret functions from \#$3$ to determine which of the linear, exponential and logarithmic functions reflect the belief that ``amount of CO2 in the atmosphere growing, but that the growth rate is speeding up." {\em Please review the Notes To Instructor in detail for this question.} There are some subtle ideas that you will want to be familiar with.
\item {\em During class} Question $5$: Students to recommend one best fit function to policy makers to predict future change of CO$_2$ in the atmosphere.
\item {\em During class} Question $6$: Student must use technology to estimate AROC by using recommended function of best fit
\item {\em During class} Question $7$: Students determine the ``tipping point" year for CO$_2$ in the atmosphere and estimate IROC using technology.
\item {\em During class} Last page of the handout contains instructions for using Desmos.
\item {\em During class} Each student team of $3$-$4$ students will submit only one final version of this handout to Gradescope.
}
An instructor version of Desmos with trendlines, estimations of average rate of change, instantaneous rate of change, the tipping point, etc. can be found at \url{https://www.desmos.com/calculator/f0b202c2cb}
\newpage
{\bf Introduction}\\
...
\nti{
This question will be asked as part of the pre-class assignment. The intention is to have students start to use Desmos before coming to class.   Instructions to create the trendline are found on last page of the both the pre-class and during-class assignments.. Here's one way to create a linear trendline.  $$c_1\sim mt_1+b$$
}
\sol{
...
\end{enumerate} 
 ...
 \nti{
 There are some subtleties involving the logarithmic trend line function.  Possible prompts:
 \itemI{
 \item Examine the graphs of each trend line.  
 \item What happens to the slope of tangent lines as $t$ grows larger? 
 \item When is a logarithmic defined? When is it not defined?
 }
 \mpageT{.7}{.3}[\hs{.2}]{
 \itemI{
 \item Tangent lines of Linear functions {\em are} the linear function itself.  This means that tangent lines for all $t$ have the same slope.  Another way to say this is that linear functions have a constant Rate of Change.  This means that the linear function we found   {\em does not} reelect this belief.
...
\itemI{
 ...
 \item But there is more going on here than is clear at first. A typical trendline of the form $f(t)=\ln( t)$ grows at a slower and slower rate (see graph on left below). Additionally  $\ds 
...
\nti{
The key support on this question is to help students think about what assumptions they are making.
}
\sol{
...
\itemI{
\item Desmos can be helpful to make this calculation
\item Try creating functions from trend line information and using them for this calculation (see last page of this handout for instructions).
}
While the students only need to utilize their recommended function, answers for all follow.\\\\
...
\newpage
 ...
 \nti{
 If students chose any recommended model besides a linear function, they likely will not have the tools to algebraically solve for the correct time.  Use of technology is highly encouraged here! Some possible prompts:
 \itemI{
 \item Utilize the graph of  your recommended function to estimate when this happens.
 \item Would graphing the function $c(t)=478$ be helpful? 
 \item Would Wolfram Alpha's solve command be helpful (ok, this means leaving the safe, beautiful confines of Desmos)?
 }
 Note: If trend line function and $c(t)=478$ are both graphed, a grey point is placed at point of intersection. Hover over this point to see details.\\
 
 
 Students are only required to use their recommended function to answer this question. I'm labeling the answer as ``$t^*$" for the purpose of utilizing this same value in next question.
 \itemI{
