...
\newcommand\TypeTitle{Introduction to Limits\\Solutions }
...
\blue{
\itemC{
\item Note that  $x=0$ and $f(0)$ do not appear in the table as we want to help you interact with idea that a limit is examination of the trend of the values of $f(x)$ as we approach $x=0$ and {\em not} what happens when $x=0$.  
\item We will come back to  to deal with ``Notation" after we talk about more formal definition of limit right before Exercise D.
}
}
\begin{center}
...
\blue{
\itemC{
\item This table now includes $x=0$ and $f(0)=3$. However, $f(0)=3$ {\em is not relevant} when finding limits.
}
}
\begin{center}
...
\blue{
\itemC{
\item This problem introduces and example  of Does Not Exist $=$ DNE.
}
}
\begin{center}
\begin{tabular}{| c || c c c c c c |}
...
\blue{
\itemC{
\item You should assume the trend of $f(x)$ continues as $x$ approaches $4$. 
\item This introduces idea that limit DNE because one sided limits approach $\pm \infty$
...
\blue{
Go back to Exercises A, B, and C and use this formal notation to write down what we did informally.
}
\vs{.3}
 \newpage
...
\nti{
This exercise uses a graph of a function to  replicate what we did informally with data in tables in Exercises A,B, and C. Fun fact: this function was used to generate the data in tables in Exercises A, B, and C!
}
%}{
