...
\def\TypeTitle{Basic Differentiation Rules\\Notes To Instructors }
...
\lt  
\enumC{
\item[\target{D1}:] Meaning of a Derivative
\itemC{
 \item Describe the relationship between the derivative and slope of the tangent line.
}
\item[\target{D3}:] Derivatives of Basic Functions
\itemC{
    \item Take the derivative using the power rule. 
    \item Take the derivative of a constant. 
    \item Take the derivative of $e^x$ 
    \item Take the derivative using the sum, difference, and constant multiple rules. 
    \item Apply algebraic simplification before taking the derivative using the power rule. 
}
}
...
\enumCN{
\item  Use algebra to convert functions involving radicals to power functions and vice versa.
\item Compute derivatives of power functions, $e^x$ and combinations of these functions.
\item Solve problems where derivatives are needed.
}

\feed
\itemC{
\item Some students appreciated the chance to practice and learn in pre-class activity.  
\item Others found this lesson clear at the start but more confusing in later part of pre-class activity.
\item Some students will be coming into class because the pre-class felt ``very difficult".  
\item {\em Note:} Most of the challenges students expressed are likely due to need for algebra to simplify function before appealing to basic derivative rules.  Here's one example mentioned by students: 
$$\text{\em Find derivative of }y=\dfrac{x^2 + x + 1}{\sqrt{x}}$$ 
}
...
\core Warmup Exercises $1$ and $2$, Exercise A and B
