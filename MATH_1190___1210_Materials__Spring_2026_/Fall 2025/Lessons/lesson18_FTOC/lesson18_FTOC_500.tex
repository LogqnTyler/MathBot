...
\blue{\bf Mini-lecture: The Fundamental Theorem of Calculus}
...
\,\\[-0.4in]
    \begin{center}
        ...
        x label style={anchor=west},
          ...
          \node at (1.5,2) {$f(x)=\sqrt{x}$};
          \end{axis}
        \end{tikzpicture}
    \end{center}
\end{minipage}\\

\example \ltrm{\bf\color{Fuchsia}I4} Calculate the value of $\ds \int_0^4 \sqrt{x}\,dx$.

\vspace{3cm}

\newpage

\exercise \ltrm{\bf\color{Fuchsia}I4} Compute the following definite integrals:
\def\vd{1}
\tasksA{2}{
\task $\dint_{-1}^1 e^x \,dx=$
\task $\dint_2^{3} \frac{1}{x^2} \,dx =$ \vs{\vd}
\task  $\dint_{-3}^{-1} 2^5 \,dx =$ 
\task $\dint_{0}^1 \frac23x^4 \,dx =$\vs{\vd}
}

\exercise \ltrm{\bf\color{Fuchsia}I4} Find the area between the graph of the curve $y=1-x^2$ and the $x$-axis.\\
...
x label style={anchor=west},
    ...
    samples     = 160,
    domain      = -1.5:1.5,
    restrict y to domain=-1:1.5,
    xmin = -1.5, xmax = 1.5,
    ymin = -0.5, ymax = 1.5,
    width = 3in, height=2in
  ]
  % piece 1
  \addplot[domain=-1.2:1.2, blue, ultra thick, <->] {1-x^2};
  \node at (1.5,2) {$f(x)=\sqrt{x}$};
  \end{axis}
\end{tikzpicture}
\end{flushright}
...
\exercise \ltrm{\bf\color{Fuchsia}I4} Suppose that a bacterial colony grows at a rate of $r(t) = 1.7^t$ bacteria per hour. If there is just one bacterium at time $t=0$, how many bacteria are there after 24 hours?

\newpage

\ltrm{\bf\color{Fuchsia}I4} 
\begin{minipage}[t]{0.6\textwidth}
    \exercise
    \begin{enumerate}
        \item[(a)] What do you think $\dint_{-1}^1 \dfrac1{x^2}\, dx$ should represent? Should this quantity be positive or negative? Can you estimate it?
...
\,\\[-0.4in]
    \begin{center}
        ...
        x label style={anchor=west},
          ...
          % piece 1
          \addplot[domain=-2.5:0.01, blue, ultra thick, <->] {1/x^2};
          % piece 2
          \addplot[domain=0.01:2.5, blue, ultra thick, <->] {1/x^2};
...
\begin{enumerate}
    \item[(b)] Find an antiderivative $F(x)$ for $f(x) = \dfrac1{x^2}$. Compute $F(x)\bigg\vert_{-1}^1$

    \vspace{1in}
    
    \item[(c)] Compare your answers from (a) and (b) with what you'd expect from the Fundamental Theorem of Calculus. Explain.
...
\tasksA{2}{
\task $\displaystyle\int_1^2 \dfrac1{x} \,dx$ \qquad FTC apply?
\task $\displaystyle\int_{-2}^{1} \sqrt{x} \,dx$ \qquad FTC apply?
}
\vs{.2}
\tasksI{2}{
\task If ``yes," \\$\displaystyle\int_1^2 \dfrac1{x} \,dx$=
\task If ``yes," \\$\displaystyle\int_{-2}^{1} \sqrt{x} \,dx=$
}
\vs{0.7}

\exercise \ltrm{\bf\color{Fuchsia}I4}  Evaluate the following integral:\\

$\dint_2^4 \dfrac{z^2-1-ze^z}{2z}\,dz$
