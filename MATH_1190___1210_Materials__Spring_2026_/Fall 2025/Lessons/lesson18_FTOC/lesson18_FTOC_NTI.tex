...
\def\TypeTitle{Fundamental Theorem of Calculus \\Notes To Instructors }
...
\lt  
\enum{
\item[I3:]Antiderivatives of Basic Functions
\itemC{
\item Identify antiderivatives of power and exponential functions.
\item Use algebraic simplification and/or properties of an indefinite integral to find the indefinite integral of a function.
}
\item[I4:] Computing Definite Integrals
\itemC{
\item Use the Fundamental Theorem of Calculus to evaluate a definite integral.
\item Identify when the Fundamental Theorem of Calculus does not apply.
}
}

\lg Students will be able to...
\enumCN{
\item Compute basic anti-derivatives
\item Identify whether or not the integrand is continuous and thus FTC applies.
\item Use the FTC to evaluate definite integrals
\item Recognized when to use algebra to simplify the integrand in order to use basic anti-derivatives and the  FTC to compute definite integrals.
\item Compute net area between the integrand and the horizontal axis by using the FTC.
}

%\feed No feedback available from F24 as pre-class work was a written assignment only.  
%\itemC{
%\item
%}
 ...
 \hand
 \itemI{
 \item We start with warm-up practice of basic indefinite integrals since students will be using these in future problems on the handout.
 \item A mini-lecture about the FTC follows. In the pre-class written assignment, students practice calculating $\ds \int_a^b f(x)\;dx$ by using areas of familiar shapes and comparing to $\Delta F=F(b)=F(a)$. This is the first formal statement of the FTC they have seen. 
 The Example/Exercise that follow the Mini-lecture is intended to draw attention to the hypothesis of the FTC that $f$ must be continuous on $[a.b]$ in order to apply the FTC. Feel free to have students work together as an exercise or lead them through this as an example. One nuance to bring out is that just because the hypothesis is not met is not the same as saying integral DNE. It only means that FTC cannot be used to determine net area.
 \item Exercise A provides practicing whether or not hypothesis of FTC is met. 
 \item Example 1a) and 1b) can be instructor-led to provide practice using algebra to simplify the integrand so that anti-derivatives of basic functions can be used along with FTC. If you choose to convert this to a student-driven Exercise, you will want to provide appropriate scaffolding to help students make progress.
 \item Exercise B provides un-scaffolded work for students that require them to a) use algebra to simplify the integrand b) find anti-derivatives of various kinds of functions c) use FTC to evaluate definite integral. {\em Please help students determine if FTC applies (or not) for each question}. Note that it's not necessary to simplify the result of FTC.
 \item Extra Practice $1$ provides more practice in the same vein as Exercise B.
 \item Extra Practice $2$ helps students distinguish between net (signed) area and area which assumes a positive result.  This is a nice problem to ask students to interact with if you have extra time.
 }

\core Exercise A, Example 1, and Exercise B.
...
\task $\dint 3^x \,dx=$ 
\sol{ $\ds \int 3^x \,dx=\dfrac{3^x}{\ln(3)}+C$}
\vs{\vd}
...
\begin{minipage}[t]{0.49\textwidth}
  ...
  \item Based on graph, should $\dint_{-2}^{1} \frac{1}{x^2}\,dx$ be positive or negative?
    \sol{Positive because on interval $[-2,1]$ all area is above horizontal axis. See blue shaded area above (this includes red shaded area).}
...
\sol{
We have a division by $0$ (i.e. discontinuity) when $z=0$ for $f(x)= \dfrac{z^2-z-1}{2z}$. But $z=0$ is not in the domain $[2,4]$ so FTC applies.
\al{
...
\al{
 ...
 &=\left(\frac{1^3}{3}+\frac{1^2}{2}+2  \right)-\left(\frac{0^3}{3}+\frac{0^2}{2}+0  \right)
