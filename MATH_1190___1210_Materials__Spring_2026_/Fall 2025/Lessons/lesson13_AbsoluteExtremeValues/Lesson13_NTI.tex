...
\def\TypeTitle{Absolute Extreme Values \\Notes to Instructors }
...
\blue{
    \textbf{A2: Lesson 13 Absolute and Extreme Values - Notes to Instructor}

\textbf{Learning Goals:}

\begin{itemize}
    \item Distinguish between local and absolute extrema.
    \item Apply derivative-based tests to locate maxima and minima.
    \item Reinforce argument flow and organization in order to reach a desired conclusion.
\end{itemize}

\textbf{Key Concepts:}
\begin{itemize}
    \item \textbf{Local Extrema:} Maximum or minimum within a neighborhood of a point.
    \item \textbf{Absolute Extrema:} Largest or smallest value of a function on a domain (may or may not coincide with local extrema).
\end{itemize}

\textbf{Tests for Extrema:}
\begin{itemize}
    \item \textbf{Extreme Value Theorem:} Any continuous function attains a max and a min on a closed interval $[a,b]$.
    \item \textbf{Closed Interval Method:}Evaluate $f(x)$ at critical points and endpoints to determine absolute extrema.
    \item \textbf{Identification of absolute extrema on intervals when endpoints are excluded}
    \item \textbf{First Derivative Test for Absolute Extrema:} Analyze the sign change of $f'(x)$ around critical points to identify maxima/minima.
\end{itemize}

\textbf{Instructor Notes:}
\begin{itemize}
    \item Highlight when local extrema are not absolute extrema.
    \item Stress the importance of endpoints and domain constraints in absolute extrema.
    \item Encourage students to interpret maxima/minima in context (e.g., size of a quantity).
\end{itemize}


\begin{itemize}
\item {\bf Warmup: Example/Exercise}  
    - Reintroduces definitions from the pre-class to help students differentiate between the idea of a local and global max/min.  
    - Four different functions are given and students are asked to identify local maxes and mins along with global maxes and mins.
    - Some recall is given to reinforce the definition of a critical point.
    - The Extreme Value Theorem is introduced. Students should read the statement carefully and determine exactly what conditions are needed for the hypothesis and how and when they can arrive at the desired conclusion.
    - Finally the closed interval method is introduced to allow students to test their reflections about the EVT an give them a method to organize the upcoming problems.
    \item {\bf Example 1:}  
    - Reinforces identification of critical numbers where $f'(x)=0$ or $f'$ DNE.  
    - Direct application of the closed interval method and how to organize the data of a differentiable function.
    
    \item {\bf Exercises A/B:}  
    - Again, gives students the opportunity to apply the closed interval method. 
    - These exercises serve to reinforce their understanding of the definition of a critical point along with domains of functions. Additionally, students are getting more practice taking derivatives.
    - Some emphasis should be placed on the flow of the solution and how we use the closed interval method as an applied tool to determine absolute extrema.
    
    \item {\bf Extrema on non-closed intervals Example/Exercise C}  
    - The concept of finding extrema on non-closed intervals is introduced. Some students may struggle with the idea that if a value is not attained within the domain of a function, then they cannot declare the next largest (resp. smallest) critical value to be the max/min.
    - Additionally, some students may struggle to grasp that, for example, the interval $(6,10]$ is not the same as the set $\{7,8,9,10\}$
    - Two graphs are given to help students explore different scenarios of when and when we might not have an absolute extrema. Care should be taken in explaining each case and the necessity of attained values.
    - Finally, students are asked to reflect on the various cases to reinforce understanding and memory.
    
    \item {\bf First Derivative Test for Absolute Extrema Exercise D}  
    - The  First Derivative Test for Absolute Extrema is introduced. Students should recognize that the uniqueness of the change in sign of the derivative can be a useful tool for determining extrema. 
    - Additionally, this problem can be used to reinforce their recall of the statement of the First Derivative Test, but, more importantly, its usage as a tool for justifying the conclusion that a function attains a max (min) at a specific critical point.
    - The general flow of the solution somewhat parallels the techniques students had previously used when working with the First Derivative Test. For example, the implementation of a sign line as a tool for calculus problems.
    
    \item {\bf Extra Practice 1/2}  
    - These problems are similar to exercises A and B with an added level of complexity in that students will need to consider the importance of the endpoints and whether or not maxes/mins are attained in the domain of definition. 
   \item {\bf Extra Practice 3}  
    - These problems are similar to example/exercise C with an added level of complexity in that students will need to consider the importance of the endpoints and whether or not maxes/mins are attained in the domain of definition. 
    - Additionally, students will need to go through the full argument of justifying extrema and their types by finding critical points, using a sign line, etc. 
    \item {\bf Extra Practice 4}  
    - This problem is little different from other problems. 
    - Students will have the opportunity to recall the domain of the log function along with the limit as $x \to 0^{+}$
    - Students will need to construct some form of argument to declare that the function is increasing and thus has a global max at $x = 3$ and why the function does not have a global minimum since there are no critical numbers in the domain of definition.
\end{itemize}
}
