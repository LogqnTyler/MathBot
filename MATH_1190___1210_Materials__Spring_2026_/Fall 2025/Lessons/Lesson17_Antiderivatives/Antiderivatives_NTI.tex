...
\enumCN{
\item understand the concept of the antiderivative of a function $f(x)$ as a function $F(x)$ whose derivative is $f(x)$
\item Learn the power rule for indefinite integrals.
\item Learn the difference between a particular and a general antiderivative
\item Learn basic antiderivatives for power functions, exponential functions and logarithms.
}


 \hand
 \itemI{
 \item Page $1$ Defines the idea of an antiderivative and its linearity.
 \item Exercise A has students find particular antiderivatives of some exponential functions by thinking about the question ``What function $F(x)$ has as its derivative $f(x)$?"
 \item Exercise B has students create a table of particular antiderivatives for some classical functions and introduces the power rule for indefinite integrals. After that, using the linearity of the antiderivative, students apply some of the rules just formulated by finding particular antiderivatives of sums of functions.
 \item Students are then asked to fill in blanks to determine some special general antiderivatives (e.g. constant functions, exponentials, power ruile and natural log) along with the linearity poroperty of antiderivatives.
 \item Exercise C  and Exam Practice 1 gives students more practice with using linearity along with the antiderivatives with which they filled in the table.
 }
...
\begin{tabular}{ r c l r c l}
$\displaystyle\int \big( f(x) \pm g(x) \big)\, dx$ & $=$ & \underline{$\textcolor{blue}{\dint f(x) dx \pm \dint g(x) dx}$} $=$ & \underline{$\textcolor{blue}{F(x) \pm G(x)}$} \text{ (where $F'(x) = f(x)$ and $G'(x) = g(x)$) }
\end{tabular}
\end{center}
...
{\it Linearity Properties of Indefinite Integrals}\\[0.1in]
\begin{tabular}{ r c l r c l}
& $\displaystyle\int kf(x) \,dx$ & $=$ & \underline{$\textcolor{blue}{k \dint f(x) dx}$} $=$ & \underline{$\textcolor{blue}{F(x)}$} \text{ (where $F'(x) = f(x)$) }
\end{tabular}
\end{center}
  \
