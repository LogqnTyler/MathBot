...
\lt  
\enumC{
\item[\target{D1}:] Meaning of  Derivative
\itemC{
    \item Describe the relationship between the derivative, slope of the tangent line, instantaneous rate of change, and average rate of change. 
    \item Explain the meaning of the derivative of a function at a given point in context, including determining the units of the derivative. 
    \item Identify and give examples of where a function might not be differentiable graphically. 
    \item Determine the sign and compare derivatives based on the graph.
}
}
...
\enum{
    \item Distinguish between an average rate of change and an instantaneous rate of change.
    \item Estimate instantaneous rate of change by using average rate of change.
 \item Construct/evaluate the derivative of a function by interpreting it in terms of the slope of the function's graph. 
    \item Identify where a function is not differentiable using its graph.
    \item Write a clause interpreting a derivative in context as an instantaneous rate of change.
    \item Identify the units of rates of change defined by a given mathematical expression.
    \item Identify when a derivative is positive, negative, or zero in a given context.
\item Create a graph of $f^{\prime}(x)$ when given a graph of $f(x)$.
}

\feed 
\itemC{
\item Many thought ideas of instantaneous rate of change and average rate of change interesting, particularly when put in a realistic context (e.g. Usain Bolt, water filling beakers, etc.)
\item Some students were interested in more practice interpreting derivatives in context and determining units.
\item A few students had questions about how limits related to derivatives.  
\item ``Why was the AROC for the flask 3.89 cm/L?"
\item ``On slide 8 I was confused about how to get the avg time and velocity, I kept doing the formula to get them but they were wrong. Could we go over how to get the avg time and velocity on short intervals?"
}

 \hand This handout is on the shorter side and will provide time to catch up if you have not finished Lesson 4.
 \itemI{
 \item Warm-up Exercise A provides practice similar to what students did in pre-class activity and gives practice interpreting Instantaneous Rate of Change (IROC) in context and interpreting IROC geometrically as the slope of a tangent line.
 \itemI{
 \item I wouldn't suggest spending too much time on plotting points in part (a) or on the table in part (b). Just fill them in before class.
    \item Sketching the secant line associated with part (c) presents a good opportunity to remind them of lesson 1 exercise C, as do parts (d) and (e). Mentioning this connection explicitly helps make the lessons feel more interconnected, which is important given students' tendency to view math as a collection of unrelated facts and algorithms. This will also help scaffold the mini-lecture.
    }
 \item Mini-lecture + Examples of When Derivatives Do Not Exist helps students connect concept of IROC with  mathematical symbols involving a limit. Note: algebraic manipulation of the limit definition of a derivative is not a Learning Target for this course.
 \itemI{
 \item The {\bf\color{blue}Mini-lecture} on page 2 is easy to sink a lot of time into, but we really don't need to spend much time here.
     \item We want to make it believable that an IROC is defined as a limit of an AROC, perhaps via annotating the secant line in the first graph.
    \item We want to make it clear that the derivative is defined by this limit, meaning the derivative thus represents IROC and slope of a tangent line.
    \item We want to establish that because limits do not always exist, derivatives do not always exist. In particular, derivatives DNE at corners because the left- and right-limits don't match, and they don't exist at vertical tangents and discontinuities generally because of an infinite limit.
 }
 \item Example 1/Exercise B provides practice determining if derivatives exist and finding values of derivatives  from graphical information. It is intended to immediately build comprehension of the contents of the mini-lecture. Here's some questions you might ask to scaffold the tasks:
 \itemI{
     \item ``According to our mini-lecture, what does the derivative $f'$ represent in terms of the graph of $f$?"
    \item ``What kind of slope does the graph of $f$ have at $x=1$? What would you say is happening to the slope of the graph of $f$ at $x=0$? What kind of slope does the graph of $f$ have at $x=0$?"
    \item ``How do you find the slope of a line?"
 }
 \item Exercise C provides practice creating graph of $f^{\prime}(x)$when given graph of $f(x)$. It is a more challenging exercise extending Exercise B. Now, students must not only infer the slopes of a function's graph, but they must also produce a sketch of the function's derivative.
 \itemI{
  \item It's easy to sink too much time into this one. Help scaffold the exercise by providing clear instructions to keep students focused on the task. Make sure they complete the table of values first, the encourage them to represent their table of values graphically.
    \item A very small number of students may have never plotted a point or sketched a curve by hand in their lives. Be ready to provide support here.
    \item Students frequently mix up height and slope when sketching the graph of $f'$. So, it may be a good idea to remind them that the height of the graph they sketch should be the same as the slope of the graph they are given.
    \item Students also forget that since the derivative DNE at $x=0$, $1$, $2$, and $4$, the graph of $f'$ should not indicate $f'$ is defined at those values.
    \item This is a great problem to have students work on in groups at the boards, or in groups at their tables. If you come into class early and sketch coordinate planes for the students beforehand, then that can make board work go a bit faster. If you have them work in groups at their tables, then you might have them sketch their group's answer on a small whiteboard or a piece of copy paper, and then share with the class using the document camera or some other means. Be sure to highlight students positive contributions, as sharing one's work with the class is rather nerveracking for many students in a math classroom.
    }
 \item Exercise D provides practice interpreting AROC and IROC in context (including finding units). It also helps connect AROC and IROC to geometric interpretation of slope of secant/tangent lines. In particular, it's an exercise in helping students (i) connect mathematics notation with AROC and IROC, (ii) interpret these things in context, and (iii) find the units of these things by noting the units of their parts.
 \itemI{
   \item We have interpreted slope before in lesson 1 as the amount the dependent quantity changes given an 1 unit increase in the independent quantity. We should try to stay consistent with that interpretation here.
    \item In both cases, the numerator is cost (\$) and the denominator is quantity of a commodity. So, the units are \$ per commodity $\left(\dfrac{\$}{\text{commodity}}\right)$. The limit does not impact the units for IROC.
    \item When interpreting AROC, students need to provide the interval across which we are averaging.
    \item When interpreting IROC, students need to provide the instant in which the rate of change occurs.
 }
 \item Extra Practice 1 helps students interpret Geogebra animation of a moving particle in terms of a derivative and determining when derivative is positive/negative/zero.
 \itemI{
  \item Students often need to view the animation several times before they understand that movement to the right indicates positive velocity and movement to the left indicates negative velocity.
    \item In part (c), students will often identify the correct instants in their head, but will use the position to describe them rather than the time.
 }
 \item Extra Practice 2 gives more practice interpreting a graph of $f(x)$ and determining the sign and magnitude of $f^{\prime}(x)$. It reinforces both interpretations of the derivative. Students usually understand to compare the tangent lines' slopes. The most common mistake is that students don't realize that quantity D is simply the number zero.

 \item Extra Practice 3 is a more challenging problem where possible graphs of $f$ and $f^{\prime}$ are given and students are asked to determine which is which.
 }

\core Example1/Exercise B, Exercise C, Exercise D.
...
You might find the Desmos activity at \href{https://student.desmos.com/}{student.desmos.com} helpful (see To Do or Past Work tab). Please use code \desmosClassCode.\\\\
...
%\end{enumerate}\end{document} %end here for written preclass
    ...
    \ep  Write down an equation for the object you sketched in part b) and graph it in Desmos. You will want to go to \href{https://student.desmos.com/}{student.desmos.com}  and use code \desmosClassCode.    \sol{
   Equation of line will be $y=62.65(t-3)+1052.44$ or $y=62.65(t-5)+1177.74$ or $y=62.65t+864.49$
    }
...
\sol{The slope of the tangent line to the graph of $f$ at the point $t=3$}
...
{\bf\color{blue} Mini-lecture.} \mod {\it Derivatives \& Differentiability}\\
