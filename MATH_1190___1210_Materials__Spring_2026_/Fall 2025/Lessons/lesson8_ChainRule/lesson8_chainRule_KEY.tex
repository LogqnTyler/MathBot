...
\def\TypeTitle{Chain Rule\\Solutions }
...
{\bf Chain Rule:} If $g$ is differentiable at $x$ and $f$ is differentiable at $g(x)$, then the composite function $f \circ g$ defined by $(f \circ g)(x)=f(g(x))$ is differentiable at $x$ and $(f \circ g)'$ is given by the formula \\\\
...
Technically, this cancellation process should be done before taking a limit and only when each function fulfills certain technical requirements. So what is above  is something that is not a proof but more of ``here's why Leibniz notation is reasonable."
}
