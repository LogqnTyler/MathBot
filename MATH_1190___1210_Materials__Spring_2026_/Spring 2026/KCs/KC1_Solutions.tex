...
\begin{enumerate}
    ...
    2 hours to 4 hours of training? Make sure to include units in your final result.\\[1cm]
    {\color{blue}
    Let $E(t)$ be the function described in the graph above.
    From 2 to 4 hours of training, the person consumes an additional \[E(4) - E(2) = 4 - 1 = 3\] hundred Calories of energy.
    }

    \vfill

    \item What is the average rate of change of total energy consumed with respect to time
    over this interval? Make sure to include units in your final result.\\[1cm]
    {\color{blue}
    \[
        \frac{E(4) - E(2)}{4-2} = \frac{3}{2} = 1.5 \quad \text{hundred Calories per hour.}
    \]
    }
    \vfill

    \item Use a sentence or two to explain the meaning of this average rate of change
    \textbf{in the context of this scenario}.\\[1cm]
    {\color{blue}
    Between 2 and 4 hours of training, the person consumed $150$ Calories per hour  on average.
    }
...
\begin{enumerate}
    ...
    {\color{blue} (the order does not matter)}\\

    i. \rule{4.7cm}{0.4pt}
    {\color{blue}$f(a)$ is defined}
    \rule{4.7cm}{0.4pt}
    \vspace{0.5cm} \\

    ii. \rule{4.65cm}{0.4pt}
    {\color{blue}$\displaystyle \lim_{x\to a} f(x)$ exists}
    \rule{4.65cm}{0.4pt}
    \vspace{0.5cm} \\

    iii. \rule{4.5cm}{0.4pt}
    {\color{blue}$\displaystyle \lim_{x\to a} f(x) = f(a)$}
    \rule{4.5cm}{0.4pt}
    \vspace{0.5cm} \\
...
\begin{itemize}
\item
$\displaystyle \lim_{x\to 2} f(x)$ DNE,
\item
...
\begin{itemize}
\item
\emph{jump discontinuity} at $x=2$,
\item
\emph{removable discontinuity} at $x=3$.
\end{itemize}}
...
\begin{enumerate}
    ...
    \begin{tabularx}{\textwidth}{lX X}
        ...
        \textbf{Form:}  {\color{blue}$\frac0c$}
    \end{tabularx}
    \smallskip
    
  {\color{blue}
  The function \( f(x) = \frac{x^2+2x-3}{x+1} \) is continuous at \( x = 1 \) by the main continuity theorem, since \( 1 \) is in the domain of \( f(x) \). Hence, we can evaluate the given limit as
\[
  \lim_{x \to 1} \frac{x^2+2x-3}{x+1} = f(1) = \frac{1+2-3}{2} = 0.
\]}
\medskip
    ...
    \begin{tabularx}{\textwidth}{lX X}
        ...
        \textbf{Value:} {\color{blue}$\frac52$} & 
        \textbf{Form:} {\color{blue}$\frac{\infty}{\infty}$}
    \end{tabularx}
\smallskip
    
    {\color{blue}
    The form of this limit is \( \frac{\infty}{\infty} \) since both the numerator and denominator grow without bound when \( x \to \infty \). The dominant terms in the numerator and denominator are \( 5x^2 \) and \( 2x^2 \) respectively. These grow at about the same rate, so we compute
\[
  \frac{5x^2}{2x^2} = \frac{5}{2},
\]
and so the value of the given limit is \( \frac{5}{2} \).}
\bigskip
...
{\color{blue}
The form of this limit is \( \frac{\infty}{\infty} \) since both the numerator and denominator grow without bound when \( x \to \infty \). The dominant terms in the numerator and denominator are \( -3e^x \) and \( \ln (x) \) respectively. The exponential \( -3e^x \) grows much faster than the logarithm \( \ln (x) \) and so the value of the limit is \( -\infty \).
}
...
\item[(a)]
{\color{blue}
The equation $P(4)=4$ means that when the sunlight intensity is 
$4\text{ kW/m}^2$, the solar panel produces $4\text{ kW}$ of power. 
In other words, at an intensity of $4\text{ kW/m}^2$, the power output of the solar panel is $4\text{ kilowatts}$.
}
\end{enumerate}
%\vfill
...
\item[(b)] 
{\color{blue}
The equation $P'(2)=1$ means that when the sunlight intensity is 
$2\text{ kW/m}^2$, the power output of the solar panel is increasing by
$1\text{ kW}$ per $\text{(kW/m}^2)$ (IROC). 
This tells us how sensitive the power output is to small changes in sunlight intensity at $I=2\text{ kW/m}^2$. For example, if  power output were to continue to change exactly as it did when the sunlight intensity was $2\text{ kW/m}^2$, $P'(2)$ is the expected increase in power output per 1 unit increase in sunlight intensity. \textbf{Note:} The units of the derivative should be $\frac{kW}{\frac{kW}{m^2}} = m^2.$ 
...
\item[(c)] 
{\color{blue}
The value of $P'(5)$ does not exist. 
From the graph, we can see that the left and right-hand limits of difference quotients (slopes of secant lines) do not agree. On teh right-hand side the limits would approach zero and on the left-hand side the limits would approach something non-zero. Alternatively, we can see that there is a corner on the graph when the sunlight intensity $I$ is $4\text{ kW/m}^2$.
}
\end{enumerate}
\end{enumerate}
