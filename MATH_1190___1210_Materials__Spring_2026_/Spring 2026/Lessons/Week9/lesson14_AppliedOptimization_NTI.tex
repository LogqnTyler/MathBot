...
\def\TypeTitle{Applied Optimization\\Notes to Instructors }
...
\lt  
\enum{
\item[A3:] Optimization
\itemC{
 \item Use the tools of calculus to find the maximum or minimum value of a quantity of interest in an applied context.
}
}

\lg Students will be able to navigate the process of creating a mathematical model and optimizing it. Students will need to...
\enumCN{
...
\feed
\itemC{
\item Some students thought pre-class was very clear. Others thought it interested but complex.  
\item Most students wanted more practice.
\item Many students struggle with ``determining which formula I should be using".
\item Some students asked for a more general outline of the process of optimizing a model function.
}

 \hand
 \itemI{
 \item The Warm-up exercise 1 on the first page is the same example as in the pre-class activity with only the volume changed from $500 cm^3$ to $256cm^3$. Thus there is only light scaffolding provided in the form of defining the variables and providing surface area and volume equations. You may need to provide some additional scaffolding for this problem.
 \item Page 2 is a reflection exercise intended on helping students to reflect on the process they used. This question exists to address some students' desire for an outline for optimizing a model function. Creating Poll Everywhere questions might be a good way to deliver these questions.
\item  Exercise A is heavily scaffolded. Many students either ignore the fact that we want to optimize Cost and focus on Surface Area only. Others ignore the area of  sides/top/bottom when calculating Cost. Scaffolding is intended to help students navigate the Modeling process to avoid these errors.
\item Exercise B is an unconstrained optimization problem and is lightly scaffolded.
\item Extra Practice 1 is an unconstrained optimization problem using the idea of {\em Marginal Cost} from economics.
\item Extra Practice 2 is a constrained optimization problem with no scaffolding. This is great practice to prep for upcoming assessment.
 }

\core Warm-up Exercise 1, Exercises A and B.
