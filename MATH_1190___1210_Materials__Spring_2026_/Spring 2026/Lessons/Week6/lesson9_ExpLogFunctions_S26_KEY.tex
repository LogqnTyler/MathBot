...
\def\TypeTitle{Properties of Exponential \& Logarithmic Functions\\Solutions }
...
\example{\target{F2}} Solve the following equations for $x$.\\
\enumA{
\item $\ds 4^{x-1}=64$\\
\sol{ 
\al{
4^{x-1}=64 & \implies \log_4 (4^{x-1})=\log_4(64)& \text{applied $\log_4$ to both sides}\\
&\implies x-1=\log(64) & \text{used log properties to simplify}\\
&\implies x-1=\log(4^3)&\text{recognized $64=4^3$}\\
&\implies x-1=3&\text{used log properties}\\
&\implies x=4
}
}
\item $\ds \log_4(x-1)-\log_4(x-4)=1$
\sol{
\al{
 \log_4(x-1)-\log_4(x-4)=1&\implies \log_4\left( \frac{x-1}{x-4}\right)=1 &\text{used log properties}\\
 &\implies 4^{\log_4\left( \frac{x-1}{x-4}\right)}=4^1 &\text{exponentiated both sides}\\
 &\implies \frac{x-1}{x-4}=4 &\text{used fact $4^x$ and $\log_4(x)$ are inverses}\\
&\implies x-1=4(x-4)&\text{multiplied both sides by $x-4$}\\
&\implies x-1=4x-16&\\
&\implies -1+16=4x-x&\\
&\implies 15=3x&\\
&\implies 5=x&
}
    The original equation involved logarithms, which are undefined when their inputs are not positive. So, we need to check whether this value  make the inputs of the logarithms negative or zero.\\
    \al{
    \log_4(x-1)-\log_4(x-4)\Big|_{x=5}=1&\implies \log_4(5-1)+\log(5-4) =1 &\text{(substituted $5$ for $x$)}\\
    &\implies \log_4(4) + \log(1)=1 &
    }
    We are {\em not} running into a problem of $\log(0)$ nor $\log(-)$ so our solution $x=5$ is good to go.
}

}
\exercise{\target{F2}} Solve the following equations for $x$.
\enumA{
\item $2^{4+2/x} = 8$\\
\sol{
\al{
2^{4+2/x} = 8 & \implies \log_2(2^{4+2/x})=\log_2(8)&\text{apply $\log_2$ to both sides}\\
& \implies 4+\frac{2}{x}=\log_2(8)&\text{used fact $2^x$ and $\log_2(x)$ are inverses}\\
& \implies 4+\frac{2}{x}=\log_2(2^3) &\text{recognized $8=2^3$}\\
& \implies 4+\frac{2}{x}=3 &\text{used log property}\\
& \implies 4-3=-\frac{2}{x} &\text{algebra}\\
& \implies 1=-\frac{2}{x} &\text{algebra}\\
& \implies x=-2 &\text{multiplied both sides by $x$}
}
}
