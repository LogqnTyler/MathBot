...
\itemC{
\item Use the derivatives of exponential and logarithmic functions in combinations of differentiation rules, such as the product rule, the quotient rule, and the chain rule. 
}
}

\enumC{
\item[D3:]  Derivatives of Basic Functions
\itemC{
 \item Take the derivatives of general exponential and logarithmic functions.
}
}
...
\enum{
\item Understand derivation of the derivative of general exponential functions, $f(x)=a^x$
\item Find derivatives of general exponential functions, $f(x)=a^x$
\item Understand derivation of the derivative of general logarithmic functions,  $\ds f(x)=\log_a(x)$.
\item Find derivatives of general logarithmic functions, $\ds f(x)=\log_a(x)$
\item Use derivatives of general exponential/logarithmic functions in context to find relevant quantities.
\item Decide which  differentiation technique should be used to start a problem involving several techniques. 
}

\feed There was no pre-class activity for this lesson last semester.


 \hand
 \itemI{
 \item Since there is no pre-class activity, we start with a Mini-lecture to derive the derivative of the general exponential function $f(x)=a^x$. This is followed by practice.
 \itemI{
 \item Example 1 and Exercise A provide practice using the Chain Rule and derivative of general exponential fuctions. One of the goals of the lesson is to help students decide how to get started. Is the structure a function-within-a-function? The product of two functions? The quotient of two functions?
 \item Exercise B provides a context to apply the derivative of a general exponential function. It's scaffolded in order to help students identify what they are asked to find and the values for parameters needed for the derivative.
 }
 \item A second Mini-lecture derives the derivative of the general logarithmic function.  
 \item Exercises C and D provide practice using the derivative of the general logarithmic function as well as identifying which technique should be used to start.
 \item Extra Practice 1 and 2 are  challenging problems. They are  great opportunities to practice  identifying which technique should be used to start the problem.
 }

\core Example 1,   Exercises A-D. The Discussion question is a great conceptual question if you have time.  Extra Practice 1 and 2 questions are also challenging problems but well worth it.  Extra Practice 3 is also a good practice of an in-context question.   
...
\nti{
One of the goals of this handout is to help students decide which of 4 approaches will be helpful to get started on the problem. The following questions might be helpful prompts for students here:
\itemI{
\item Is this one of our basic derivatives?
\item Is this a function-within-a-function?
\item Is this the product of two functions?
\item Is this the quotient of two functions?
}
}
\sol{
...
\nti{
The same prompts suggested in Example 1 might be helpful here.
}
\sol{
...
\nti{
Students might need prompting to see the  function-within-a-function and thus to use the Chain Rule.
}
\sol{
...
\nti{
Students find this question challenging to start. I'm going to prompt students to think about the structure of the function:
\itemI{
\item Can the function be re-written as a function-in-a-function such as $f(g(x))$?
\item Can the function be re-written as the product of two functions $f(x)\cdot g(x)$?
\item Can the function be re-written as the quotient of two functions $\ds \frac{f(x)}{ g(x)}$?
}
}
\sol{
...
\sol{
I disagree! We would need to use the Chain Rule and would get $$g^{\prime}(x)=\frac{d}{dx} \ln(f(x))=\frac{1}{f(x)}\cdot f^{\prime}(x) $$
}
