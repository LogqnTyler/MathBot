...
\exercise{\color{RedOrange}A1}  Pictured to the right is the graph of a function $f(x)$.
\begin{enumerate}
\item[(a)] What are the critical numbers of $f$?
\vspace{1.5cm}
\item[(b)] On which intervals is the function $f$ be decreasing?
\end{enumerate}
\end{minipage}
%
...
\end{minipage}\\[40pt]
\begin{minipage}{0.55\textwidth}
Now pictured is the graph of the \textit{derivative} $g'(x)$ of a different function. Use this to infer information about $g(x)$.
\begin{enumerate}
\item[(c)] What are the critical numbers of $g$?
\vspace{1.5cm}
\item[(d)] On which intervals is the function $g$ be decreasing?
\end{enumerate}
\end{minipage}
%
\begin{minipage}{0.45\textwidth}
\begin{center}
    \begin{tikzpicture}[scale=1]
          ...
          %new version of the curve in pieces
          %
          \addplot[domain=-1:3, ultra thick, red,<-] {(x-2)^2-2};
          \addplot[domain=3:4, ultra thick, red] {-(x-4)^2};
          \addplot[domain=4:5, ultra thick, red] {-2*(x-4)^2};
          \addplot[domain=5:9, ultra thick, red,->] {2*(x-6)^2-4};
          %
        \end{axis}
    \end{tikzpicture}
\end{center}
