...
\hrulefill\\
{\bf \blue{Question:}} What are some ways a function $f(x)$ can behave at a point $x=a$? 
...
In the pre-class assignment, you saw the definition of continuity, as well as different types of discontinuities. The goal of this exercise is to learn how to use the definition of continuity to decide whether a function is continuous at a number and to discern between different types of discontinuities.\\
...
\bluebf{Key Question $\mathbf{1}$: When is a function {\em not} continuous?}\\
...
\itemC{
\item $f$ has a {\bf removable} discontinuity at $a$ {\it always} means continuity condition(s) \unline{.75}{}{.125} are violated.\\\\
\item $f$ has a {\bf jump} discontinuity at $a$ {\it always} means continuity condition(s)\unline{.75}{}{.125} is/are violated.\\\\
\item $f$ has an {\bf infinite} discontinuity at $a$ {\it always} means continuity condition(s) \unline{.75}{}{.125} is/are violated.\\\\
}
}
\vs{-.3}
\bluebf{Key Implication of Continuity}\vs{.5}\\
\mpageT{.5}{.5}[\hs{.15}]{
...
\bluebf{Key Question $\mathbf{2}$: When can we be confident that a function {\em is} continuous?} The pre-class assignment for today's lesson covered the Main Continuity Theorem and the Rules for Finding Domains:
%
...
\vs{.3}\\
is continuous on it's \unline{1}{}{.15}.
%
}
...
\mpageT{.8}{.3}{
\begin{itemize}
\item{\bf Rule \#1:}  \\%Division by $0$ 
%\item{\bf Rule \#2:} \\ %Even roots of negative numbers
%\item{\bf Rule \#3:}\\ % Log of $0$ or negative number
\end{itemize}
}{
