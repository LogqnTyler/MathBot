...
\def\TypeTitle{Limits and Continuity Solutions \\Key}
...
%\vs{-.2}
  ...
  \centering {\bf Notation/Observations}
    \blue{
  \itemI{
  \item$\ds \lim_{x\to a^-}f(x)=4$
  \item$\ds \lim_{x\to a^+}f(x)=4$  \vs{.1}
    \item$\ds \lim_{x\to a}f(x)=4$  
...
~\\
\bluebf{Key Implication of Continuity}\\
\blue{Continuity allows us to evaluate limits!  $\ds \lim_{x\to a}f(x)=f(a)$}\\
\mpageT{.5}{.5}[\hs{.15}]{
...
\vs{.1}\\
is continuous on it's \unline{1}{\blue{domain}}{.15}.
%
}
