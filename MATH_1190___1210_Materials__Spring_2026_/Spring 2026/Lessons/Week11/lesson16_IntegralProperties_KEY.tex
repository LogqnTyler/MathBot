...
\def\TypeTitle{Properties of Definite Integrals\\Key  }
...
\sol{Graphing  $f(x)=1-|x|$ can be accomplished by evaluating $f(x0)$ and plotting points.  Alternatively, we can start by 1) graphing $y=|x|$, 2) Graphing $y=-|x|$ which results in graph of $y=|x|$  flipped upside down. 3) Graphing $1-|x|$ by translating $y=-|x|$ up one unit.\\\\
After graphing  $f(x)=1-|x|$ on $[-1,2]$, we  note that the region above the $x$-axis on $[-1,1]$ will be positive and the region below the $x$-axis on $[1,2]$ will be negative;
\itemI{
...
\sol{Area above the $x$-axis on $[2,4]$ is positive. We have a half-circle with \\
