...
async def query_chunks(request: QueryRequest):
    try:
        ...
        # Build keyword array for postgres && operator
        keyword_conditions = " OR ".join([
            f"keywords && ARRAY[%s]::varchar[]"
        ...
        params = (
            ...
            search_terms * 3 +                      # text search LIKE
...
async def get_kinds():
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT kind, COUNT(*) as count FROM chunks GROUP BY kind ORDER BY count DESC")
    rows = cursor.fetchall()
    cursor.close()
    conn.close()
